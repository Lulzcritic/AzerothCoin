...
#if defined(NDEBUG)
# error "AzerothCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1dcd695782d53388f44629cbfee94f0a10bff3ac76f8af802d4be9d2c03fae3c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AzerothCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AzerothCoin Signed Message:\n";
...
{
    // AzerothCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // AzerothCoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // AzerothCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // AzerothCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AzerothCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AzerothCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xaadae5220deb31d090da47fe237b86ed27b0b2b55075c2ed9f7242584c2cf0c1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "19/08/2020 Exit the rat race";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043ffdc8fc24c70be9b8ec306a1e89e801ae820e1c3084f6deb0b91d51c83e01064b3c635f3dfeec423519a2503a55fcf1ade880485d22fc961cdb8daffe317831") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1597857876;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084930688;

        if (fTestNet)
        {
            block.nTime    = 1597857808;
            block.nNonce   = 386171974;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x44ab746bd7855aafa1b0f800d8cb584b317e30e2bc856184c493e96cf31af67b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xc3, 0xb3, 0xd1 }; // AzerothCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AzerothCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AzerothCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AzerothCoinMiner(CWallet *pwallet)
{
    printf("AzerothCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("AzerothCoin-miner");
    ...
    try { loop {
        ...
        printf("Running AzerothCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AzerothCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AzerothCoinMiner, pwallet));
}
